...
(function ( $ ) {
    $.fn.omniva_global = function(options) {
        var settings = $.extend({
            maxShow: 8,
            showMap: true,
        }, options );
        ...
        var cookie_terminal = omniva_getCookie('omniva_global_terminal');
        /*
        ...
        */
        ...
        search.on('selectpostcode',function(){
            var autoselect = omnivaSettings.auto_select;
        ...
        function refreshList(autoselect){        
            ...
            map.eachLayer(function (layer) {
                if (layer.options.terminalId !== undefined){
            ...
            $(terminals).each(function(i,val){
                ...
                li.html(val[0]);
                if (val['distance'] !== undefined && val['distance'] != false && (omnivaSettings.max_distance === 0 || omnivaSettings.max_distance > val['distance'])){
                ...
                } else if (omnivaSettings.max_distance === 0 ) {
                    if (settings.showMap == true ){
                ...
                if (val['distance'] !== undefined && val['distance'] != false && (omnivaSettings.max_distance === 0 || omnivaSettings.max_distance > val['distance'])) {
                    ...
                    L.marker([val[1], val[2]], {icon: terminalIcon, terminalId:val[3] }).on('click',function(e){ listTerminals(locations,0,this.options.terminalId);terminalDetails(this.options.terminalId);}).addTo(map);
            ...
            if ( autoselect == true && ($('#shipping_postcode').val() || $('#billing_postcode').val()) ){
                var first = list.find('li:not(.city):first');
                if (cookie_terminal && list.find('li:not(.city)[data-id="'+cookie_terminal+'"]').length > 0 ){
                    first = list.find('li:not(.city)[data-id="'+cookie_terminal+'"]:first');
                }
                list.find('li').removeClass('selected');
        ...
        function selectOption(option){
            ...
            omniva_setCookie('omniva_global_terminal', option.attr('data-id'), 30);
        ...
        function initMap(){
          ...
          var Icon2 = L.Icon.extend({
            ...
            terminalIcon = new Icon({iconUrl: omnivadata.omniva_plugin_url+'assets/images/sasi.png'});
            homeIcon = new Icon2({iconUrl: omnivadata.omniva_plugin_url+'assets/images/locator_img.png'});
        ...
        function terminalSelected(terminal,close) {
          if (close === undefined){
              ...
              $('select[name="omniva_global_terminal"]').val(terminal);
              $('select[name="omniva_global_terminal"]').trigger("change");
              if (close){
                $('#omnivaLtModal').hide();
            }
        }
